...
const DailyQuizScreen: FunctionComponent<props> = ({navigation, route}) => {
  const [load, setLoad] = useState(true);
  const [error, setError] = useState('');
  const [isRefreshing, setRefreshing] = useState(false);

  const subject_tests: Array<any> = useSelector(
    (state: any) => state.main.subject_tests,
  );
  ...
  const get_data = useCallback(async () => {
    ...
    try {
      //* fetch Tests
      route.params.type === 'SUBJECT'
        ? await dispatch(getSubjectQuizzes(route.params.subject))
        : await dispatch(getQuizzes(route.params.category));
    } catch (err) {
